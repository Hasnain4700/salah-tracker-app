...
self.lastAdhanNotified = ""; // Persistence key for deduplication
...
async function saveSyncedData(prayers, struggle, logs) {
  try {
    ...
    const data = {
      ...
      struggle,
      logs,
      lastAdhanNotified: self.lastAdhanNotified,
      timestamp: Date.now()
    };
...
async function loadSyncedData() {
  try {
    ...
    if (response) {
      ...
      userLogs = data.logs || {};
      self.lastAdhanNotified = data.lastAdhanNotified || "";
      console.log('[FCM SW] State restored from cache. Last notified:', self.lastAdhanNotified);
      return true;
    }
  } catch (e) {
...
async function updateStickyNotification() {
  ...
  const dateKey = getTodayDateString(now);
  const todayLogs = userLogs[dateKey] || {};
  ...
  if (nextIndex > 0) {
    ...
    const prevKey = `${prevPrayer.name}_${dateKey}`;
    if (diffPrev >= 0 && diffPrev <= NOTIFICATION_WINDOW_MINS && self.lastAdhanNotified !== prevKey) {
      ...
      saveSyncedData(prayerTimes, strugglePrayer, userLogs); // Persist immediately after triggering
    }
    ...
    const lastKey = `${lastPrayer.name}_${dateKey}`;
